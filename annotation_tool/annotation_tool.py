...
import numpy as np
from ellipses import (  # The code is pulled from https://github.com/bdhammel/least-squares-ellipse-fitting
    LSqEllipse,
)
...
def fit_pupil(image_path, curr_image_number, plot=False, write_annotation=False):
    while True:
        ...
        ax.set_title(f"({curr_image_number}): {os.path.basename(image_path)}")
        ax.imshow(img, cmap="gray")

        key_points = plt.ginput(
            -1, mouse_pop=2, mouse_stop=3, timeout=-1
        )  # If negative, accumulate clicks until the input is terminated manually.
        ...
        if not key_points:
            if write_annotation:
                ...
                with open(
                    os.path.splitext(image_path)[0] + "_points.txt", "w+"
                ) as f:  # For detecting selected
                    f.write("closed_eye")
            plt.close()
            break
        ...
        if write_annotation:
            with open(_get_annotation_path_from_image_path(image_path), "w+") as f:
                if all([c <= 50 for c in center_coord]):
                    ...
                    points_str = f"{center_coord[0]}:{center_coord[1]}"
                f.write(points_str)
