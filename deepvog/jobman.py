...
class deepvog_jobman_CLI(object):
    ...
    def fit(
        self, vid_path, output_json_path, output_video_path="", heatmap=False, print_prefix=""
    ):
...
class deepvog_jobman_table_CLI(deepvog_jobman_CLI):
    def __init__(
        ...
        self.skip_errors, self.skip_existed, self.error_log_path = (
            skip_errors,
            skip_existed,
            error_log_path,
        )
        super(deepvog_jobman_table_CLI, self).__init__(
            gpu_num, flen, ori_video_shape, sensor_size, batch_size
        )
        ...
        for i in range(num_operations):
            try:
                ...
                if self.skip_existed:
                    output_existed, concerned_path = self._check_if_output_existed(
                        current_operation, i
                    )
                    if output_existed:
                        print(
                            "Video %d skipped (operation %s),\nbecause %s is/are found"
                            % (i + 1, current_operation, str(concerned_path))
                        )
                        continue
                ...
                if self.skip_errors:
                    try:
                        print(f"\nError encountered. Video {i + 1} skipped.")
        ...
        if self.error_log_path:
            ...
            with open(self.error_log_path, "w") as fh:
                ...
                content = "Configurations:\nFLEN:{}\nOriginal video shape:{}\nSensor size:{}\nBatch size:{}\n".format(
                    self.flen, self.ori_video_shape, self.sensor_size, self.batch_size
                )
    ...
    def _log_error(self, i):
        ...
        with open(self.error_log_path, "a") as fh:
            msg = [f"{k}: {v[i]}(delimiter)" for k, v in self.csv_dict.items()]
            msg_pretty = "".join(msg).replace("(delimiter)", "\n")
            line_separator = "\n" + "=" * 30 + "\n"
            fh.write(line_separator + msg_pretty)
            traceback.print_exc(file=fh)
        print(f"Error logged in {self.error_log_path}")
